...
const experienceData = window.KCA_EXPERIENCES || [];
...
function renderExperiences(){
  ...
  requestAnimationFrame(()=>{updateExperienceArrows(); updateActiveExperienceCard();});
}
...
function updateActiveExperienceCard(){
  ...
  scroller.querySelectorAll('.experience-card').forEach(card=>{
    const rect=card.getBoundingClientRect();
    const d=Math.abs((rect.left+rect.width/2)-centre);
